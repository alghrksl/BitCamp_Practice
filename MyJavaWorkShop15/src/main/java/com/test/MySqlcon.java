...
public class MySqlcon {
	public static void main(String args[]) {
		List<Employee2> emps = new ArrayList<Employee2>();
		try {
			Class.forName("com.mysql.jdbc.Driver");
			Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/employees", "test", "1234");
			Statement stmt = con.createStatement();
			ResultSet rs = stmt.executeQuery("SELECT emp_no,first_name, gender FROM employees.employees;");
			while (rs.next()) {
//				System.out.println(rs.getInt(1)+"  "+rs.getString(2)+"  "+rs.getString(3));
				emps.add(new Employee2(rs.getInt(1), rs.getString(2), rs.getString(3)));
				;
			}
			con.close();
		} catch (Exception e) {
